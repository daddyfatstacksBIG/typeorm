...
As you can see in this example we did not call save or softRemove for category1
and category2. But They will be automatically saved and soft-deleted when the
cascade of relation options is set to true like this:
...
`Post` and `Category` to have a many-to-many relationship with additional
`order` column, you need to create entity `PostToCategory` with two `ManyToOne`
...
export class PostToCategory {
    ...
    @ManyToOne(
        type => Post,
        post => post.postToCategories
    )
    public post!: Post;

    @ManyToOne(
        type => Category,
        category => category.postToCategories
    )
